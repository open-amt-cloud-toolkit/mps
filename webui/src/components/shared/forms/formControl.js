...
export class FormControl extends Component {
  constructor(props) {
    ...
    this.input = null; // ref of the underlying DOM element. myRef callback will set this when called by React.
    this.myRef = el => this.input = el // for uncontrolled components like fileinput
  }

  onChange = (evt) => {
    const { onChange, link } = this.props;
    if (link && isFunc(link.onChange)) link.onChange(evt, this.input);
    if (isFunc(onChange)) onChange(evt, this.input);
  ...
  selectControl(type, controlProps) {
    switch(type) {
      case 'file':
        const { value, ...newControlProps} = controlProps // filterout the value...we cannot set it programmatically.
        return <input type="file" {...newControlProps} ref={this.myRef} multiple/>;
      case 'text':
