...
const Common = {
  ...
  ReadInt: (v: string, p: number): number =>
    v.charCodeAt(p) * 0x1000000 + (v.charCodeAt(p + 1) << 16) + (v.charCodeAt(p + 2) << 8) + v.charCodeAt(p + 3), // We use "*0x1000000" instead
  ReadIntX: (v: string, p: number): number =>
    v.charCodeAt(p + 3) * 0x1000000 + (v.charCodeAt(p + 2) << 16) + (v.charCodeAt(p + 1) << 8) + v.charCodeAt(p),
  ...
  Hex2rstr: (d: string): string => {
    ...
    const m = ('' + d).match(/../g)
    for (const char of m) {
      r += String.fromCharCode((('0x' as any) + char) as any)
    }
    return r
  },
  ...
  GetSidString(sid: string): string {
    let value = `S-${sid.charCodeAt(0)}-${sid.charCodeAt(7)}`
