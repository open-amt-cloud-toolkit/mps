...
export async function powerState (req: Request, res: Response): Promise<void> {
  try {
    ...
    if (ciraconn && ciraconn.readyState === 'open') {
      ...
      await MqttProvider.publishEvent('request', ['AMT_PowerState'], 'Power State Requested', guid)

      amtstack.Enum('CIM_ServiceAvailableToElement', async (stack, name, responses, status) => {
        stack.wsman.comm.socket.sendchannelclose()
        if (status !== 200) {
          await MqttProvider.publishEvent('fail', ['AMT_PowerState'], 'Failed to Get Power State', guid)
          ...
          await MqttProvider.publishEvent('success', ['AMT_PowerState'], 'Sent Power State', guid)
          res.status(200).json({ powerstate: responses[0].PowerState }).end()
        }
      })
    } else {
      await MqttProvider.publishEvent('fail', ['AMT_PowerState'], 'Device Not Found', guid)
      res.status(404).json(ErrorResponse(404, `guid : ${guid}`, 'device')).end()
    }
  } catch (error) {
    log.error(`Exception in Power state : ${error}`)
    await MqttProvider.publishEvent('fail', ['AMT_PowerState'], 'Internal Server Error')
