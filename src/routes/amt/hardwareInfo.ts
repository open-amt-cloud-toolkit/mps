...
export async function hardwareInfo (req: Request, res: Response): Promise<void> {
  try {
    ...
    if (ciraconn && ciraconn.readyState === 'open') {
      ...
      await MqttProvider.publishEvent('request', ['AMT_HardwareInfo'], 'Hardware Information Requested', guid)
      ...
      async (stack, name, responses, status) => {
        ...
        if (status !== 200) {
          ...
          await MqttProvider.publishEvent('fail', ['AMT_HardwareInfo'], 'Failed to Get Hardware Information', guid)
          ...
          await MqttProvider.publishEvent('success', ['AMT_HardwareInfo'], 'Sent Hardware Information', guid)
          res.status(status).json(responses).end()
        }
      })
    } else {
      await MqttProvider.publishEvent('fail', ['AMT_HardwareInfo'], 'Device Not Found', guid)
      res.status(404).json(ErrorResponse(404, `guid : ${guid}`, 'device')).end()
    }
  } catch (error) {
    log.error(`Exception in AMT HardwareInformation : ${error}`)
    await MqttProvider.publishEvent('fail', ['AMT_HardwareInfo'], 'Interanl Server Error')
