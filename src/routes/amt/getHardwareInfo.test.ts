...
describe('Hardware information', () => {
  ...
  it('should get hardware information', async () => {
    ComputerSystemPackageSpy.mockResolvedValueOnce(computerSystemPackage)
    ChassisSpy.mockResolvedValueOnce(chassis)
    CardSpy.mockResolvedValueOnce(card)
    BIOSElementSpy.mockResolvedValueOnce(biosElement)
    ProcessorSpy.mockResolvedValueOnce(processor)
    PhysicalMemorySpy.mockResolvedValueOnce(physicalMemory)
    MediaAccessDeviceSpy.mockResolvedValueOnce(mediaAccessDevice)
    PhysicalPackageSpy.mockResolvedValueOnce(physicalPackage)
    SystemPackagingSpy.mockResolvedValueOnce(systemPackaging)
    ChipSpy.mockResolvedValueOnce(chip)
