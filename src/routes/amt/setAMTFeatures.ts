...
export async function setAMTFeatures (req: Request, res: Response): Promise<void> {
  try {
    ...
    if (ciraconn && ciraconn.readyState === 'open') {
      ...
      req.mpsService.mqtt.message({ type: 'request', method: 'AMT_SetFeatures', guid, message: 'Set AMT Features Requested' })

      await AMTFeatures.setAMTFeatures(amtstack, payload)
      amtstack.wsman.comm.socket.sendchannelclose()

      req.mpsService.mqtt.message({ type: 'success', method: 'AMT_SetFeatures', guid, message: 'Set AMT Features' })
